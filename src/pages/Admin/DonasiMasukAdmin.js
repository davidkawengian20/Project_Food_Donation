...
function DonasiMasukAdmin({route, navigation}) {
  ...
  const [melebihi, setMelebih] = useState('');
      ...
      .then(textData => {
        ...
        if (textData.includes('SUCCESS')) {
          ...
          const familyNames = jsonData.map(item => ({
            label: `${item.Family_Name}  ${item.jumlah_makanan_diterima}x`,
            value:
              item.jumlah_makanan_diterima >= 2 ? 'tidak bisa' : item.Family_ID,
          }));
  ...
  const handleConfirm = async () => {
    ...
    if (value === 'tidak bisa') {
      Alert.alert(
        'Keluarga Melebihi batas',
        'Keluarga yang dipilih melebihi batas Donasi Silahkan coba lagi nanti.',
      );
      return;
    }
          ...
          <ScrollView>
            ...
            <View style={styles.itemContainer}>
              <View style={styles.itemDetails}>
                ...
                <TouchableOpacity onPress={foto} style={{marginRight: 263}}>
                  <Image
